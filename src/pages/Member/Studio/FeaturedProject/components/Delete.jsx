...
function Delete(props) {
  const { oldFeaturedProject } = props;

  const featuredProject = oldFeaturedProject;
  ...
  const handleDelete = () => {
    deleteFileFromStorage(featuredProject.media.fileName)
      .then((res) => {
        return axios.delete(`${apiServer}/projects/${featuredProject.id}`);
      })
      .then((res) => {
        alert.success("Successfully delete your project");
        navigate(
          `/${featuredProject.Studio.SmartistsUser.walletAddress}/studio/project`
        );
      })
      .catch((err) => {
